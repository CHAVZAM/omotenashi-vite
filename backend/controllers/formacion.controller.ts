...
export const getProgreso = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error interno del servidor al cargar el progreso." });
  }
};
...
export const completarNivel = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId || !nivel) {
      return res
        .status(400)
        .json({ message: "Datos de progreso incompletos." });
    }
    ...
    res
      .status(500)
      .json({ message: "Error interno del servidor al guardar el progreso." });
  }
};
